...
def knights_tour_hbo(n, m, num_bees=10, num_iterations=100):
    """
    Function to solve the Knight's Tour problem on an n x m chessboard
    with visual representation using Honey Bee Optimization (HBO)
    """
    ...
    # Initialize bees with random starting positions
    bees = []
    for _ in range(num_bees):
        start_x, start_y = random.randint(0, n-1), random.randint(0, m-1)
        bees.append((start_x, start_y))

    global_best_position = None
    global_best_cost = float('inf')

    for _ in range(num_iterations):
        for i, (current_x, current_y) in enumerate(bees):
            # Initialize board and move count for this bee
            ...
            while move_count < n * m:
                ...
                # Choose next move randomly for HBO simplicity
                next_x, next_y = random.choice(possible_moves)
                board[next_x][next_y] = move_count
                current_x, current_y = next_x, next_y
                move_count += 1

            # Evaluate cost (number of moves) for this bee's tour
            cost = np.sum(board)  # Sum of all elements in board is the cost

            # Update global best if this bee found a better solution
            if cost < global_best_cost:
                global_best_cost = cost
                global_best_position = bees[i]

            # Clear the board for the next iteration
            board = [[-1 for _ in range(m)] for _ in range(n)]

        # Update bees positions (randomly)
        for i in range(num_bees):
            bees[i] = (random.randint(0, n-1), random.randint(0, m-1))
...
# Example usage:
knights_tour_hbo(8, 8, num_bees=10, num_iterations=20)
